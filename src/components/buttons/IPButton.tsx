...
const IPButton: FC<ButtonProps> = ({ tooltipmssg = "Administrar IPs", loading = false, ...props }) => {
    return (
        <ToolTip message={tooltipmssg} placement="top-start">
            <button
                {...props}
                className='btn btn-icon btn-light btn-active-color-primary btn-sm btn-transparent'
                disabled={loading}
            >
                {loading ?
