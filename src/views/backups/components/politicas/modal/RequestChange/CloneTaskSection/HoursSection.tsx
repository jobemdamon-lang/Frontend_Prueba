...
const HoursSection: FC<Props> = ({ taskCloneFunctions }) => {
  ...
  useEffect(() => {
    const formatterData: IDataTableRowsHours[] = taskCloneFunctions.clonedTask.lista_hora?.map((hour: ICreateHours) => {
  ...
  }, [taskCloneFunctions.clonedTask.lista_hora])
  ...
  const addHourToTable = (selectedHour: string) => {
    ...
    taskCloneFunctions.updateAgregarHora(newHour)
  }

  //Funcion para eliminar una fila de la tabla
  const handleDelete = () => {
    setToggleCleared(!toggleCleared);
    taskCloneFunctions.updateEliminarHora(selectedRows)
  }
