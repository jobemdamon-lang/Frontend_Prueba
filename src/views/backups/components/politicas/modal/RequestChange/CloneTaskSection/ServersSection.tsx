...
const ServersSection: FC<Props> = ({ serversData, serversCombo, taskCloneFunctions }) => {
  ...
  useEffect(() => {
    const serverWithFormat: IDataTableRowsServers[] = taskCloneFunctions.clonedTask.lista_server?.map((server: ICreateServers) => {
  ...
  }, [taskCloneFunctions.clonedTask.lista_server, serversData])

  //Eliminar los o el CI seleccionado de la tabla
  const handleDelete = () => {
    setToggleCleared(!toggleCleared);
    taskCloneFunctions.updateEliminarServidor(selectedRows)
  }
  ...
  const addServerToTable = (selectedServer: string) => {
    ...
    taskCloneFunctions.updateAgregarServidor(serverToAdd)
  }

  return (
    <div className="d-flex flex-column p-8 gap-5">
      <div className="d-flex justify-content-around align-items-end">
        <ComboBoxInput label="Servidor" data={serversCombo} setNewValue={setSelectedServer} value={selectedServer}/>
