...
const FrecuencySection: FC<Props> = ({ taskCloneFunctions, ComboData }) => {
  ...
  useEffect(() => {
    const formatterData: IDataTableRowsFrecuencyProtection[] = taskCloneFunctions.clonedTask.lista_proteccion_frecuencia?.map((freqProt: ICreateFrecuencyProtection) => {
      const data: IDataTableRowsFrecuencyProtection = { ...freqProt }
      return data
    })
    setFrecuencyProtection(formatterData)
  }, [taskCloneFunctions.clonedTask.lista_proteccion_frecuencia])
  ...
  const addFrecuencyProtectionToTable = (selectedFrecuency: string, selectedProtection: string) => {
    if (selectedFrecuency === "" || selectedProtection === "") {
      ...
      taskCloneFunctions.updateAgregarFrecuenciaProteccion(newCrecuencyProtection)
    }
  }
  ...
  const handleDelete = () => {
    ...
    taskCloneFunctions.updateEliminarFrecuenciaProteccion(selectedRows)
  }

  //Funcion para actualizar una fila de la tabla
  /*   function handleUpdateHour(id_row_hora: string, newHour: string) {
      taskCloneFunctions.updateModificarHora(id_row_hora, newHour)
    } */
