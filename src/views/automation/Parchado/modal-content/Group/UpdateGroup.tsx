...
const UpdateGroup: FC<Props> = ({ OPERATE_SYSTEM_ENV }) => {
    ...
    const { groupHook, templateHook, serverHook, selectedOwners, modalHook } = useWindowsPatchContext()
    ...
    const handleDeleteGroup = (idServer: number) => {
        const group = groupHook.groupsData.find(group => group.NOMBRE === groupName)
        if (!group) return;
        groupHook.deleteServerInGroup(group.ID_GRUPO, idServer).then(success => {
            if (success) {
    ...
    const handleChangeNameGroup = () => {
        ...
        AnalyticsService.event("change_group_name", { module: "parchado" });
    ...
    return (
        <div className='d-flex flex-column gap-5'>
                ...
                <div className="d-flex gap-5 align-items-end">
                    ...
                    <button
                        ...
                        onClick={() => {
                            ...
                            AnalyticsService.event("reassign_template", { module: "parchado" });
                    ...
                    >
                        {groupHook.reassignTemplateOfGroupLoading ? "Reasignando..." : "Reasignar Plantilla"}
                    </button>
                    <button
            ...
            <div className="position-relative">
                <DataTable
                    columns={UpdateGroupColumns(handleDeleteGroup, groupHook.deleteServerInGroupLoading)}
                    pagination
                    highlightOnHover
                    persistTableHead
...
const UpdateGroupColumns = (handleDeleteGroup: (idServer: number) => void, deleteServerInGroupLoading: boolean): TableColumn<IListServerUnified>[] => [
    {
        name: 'Nombre del CI',
